...
export function DroneListSidebar({
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`sidebar bg-card border-r border-border w-80 flex flex-col fixed md:relative h-full z-40 md:z-auto ${isOpen ? 'open' : ''}`}>
        
        {/* Header */}
        <div className="p-6 border-b border-border">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-bold text-foreground">SagerSpace</h1>
              <p className="text-sm text-muted-foreground">Drone Tracking System</p>
            </div>
        ...
        <div className="p-6 border-b border-border">
          <div className="grid grid-cols-2 gap-4">
            <Card className="bg-muted/50 p-3">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs text-muted-foreground uppercase tracking-wide">Total Drones</p>
                  <p className="text-xl font-bold text-foreground" data-testid="text-total-drones">
                    {droneStats.total}
                  </p>
                </div>
            ...
            <Card className="bg-muted/50 p-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xl font-bold text-chart-2" data-testid="text-active-drones">
                    {droneStats.allowed}
                  </p>
                </div>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            <div className={`p-2 rounded-full ${isAllowed ? 'bg-chart-2/20' : 'bg-destructive/20'}`}>
                              <Plane className={`h-3 w-3 ${isAllowed ? 'text-chart-2' : 'text-destructive'}`} />
                            </div>
                            <div>
                              <p className="font-medium text-foreground text-sm" data-testid={`text-drone-registration-${drone.id}`}>
                                {drone.registration}
                              </p>
                              <p className="text-xs text-muted-foreground" data-testid={`text-drone-flight-time-${drone.id}`}>
                                Flight: {formatFlightTime(drone.flightTime)}
                              </p>
                            </div>
                          </div>
                          <div className="text-right">
                            <p className={`text-sm font-medium ${isAllowed ? 'drone-allowed' : 'drone-restricted'}`}>
                              {isAllowed ? 'ALLOWED' : 'RESTRICTED'}
                            </p>
                            <p className="text-xs text-muted-foreground" data-testid={`text-drone-altitude-${drone.id}`}>
